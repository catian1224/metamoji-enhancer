...
(function () {
	...
	const ClassroomTextTransform = (() => {
	    ...
	    function invertCharacterMap(characterMap) {
	        return Object.fromEntries(
	            Object.entries(characterMap).map(([normal, transformed]) => [
	                transformed,
	                normal,
	            ]),
	        );
	    ...
	    const normalMaps = {
	        superscript: invertCharacterMap(characterMaps.superscript),
	        subscript: invertCharacterMap(characterMaps.subscript),
	    };

	    function transformCharacters(text, characterMap) {
	        return Array.from(
	            text,
	    ...
	    function normalize(text) {
	        return Array.from(
	            text,
	            (character) =>
	                normalMaps.superscript[character] ||
	                normalMaps.subscript[character] ||
	                character,
	        ).join("");
	    }
	    ...
	    function getMode(text) {
	        ...
	        for (const mode of ["superscript", "subscript"]) {
	            const otherMode =
	                mode === "superscript" ? "subscript" : "superscript";
	            let transformedCharacterFound = false;
	            let matchesMode = true;

	            for (const character of characters) {
	                if (normalMaps[mode][character]) {
	                    transformedCharacterFound = true;
	                    continue;
	                }
	                if (
	                    normalMaps[otherMode][character] ||
	                    characterMaps[mode][character]
	                ) {
	                    matchesMode = false;
	                    break;
	                }
	            ...
	            if (matchesMode && transformedCharacterFound) {
	                return mode;
	            }
	        }
	        return null;
	    ...
	    function transform(text, targetMode) {
	        const normalizedText = normalize(text);
	        if (getMode(text) === targetMode) {
	            return normalizedText;
	        }
	        return transformCharacters(normalizedText, characterMaps[targetMode]);
	    }
